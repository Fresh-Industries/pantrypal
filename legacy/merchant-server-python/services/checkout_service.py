...
class CheckoutService:
    ...
    try:
      for line in checkout.line_items:
        ...
        if await db.get_product(self.products_session, product_id):
          ...
          qty_avail = await db.get_inventory(
            self.transactions_session, product_id
          )
          if qty_avail is None:
            fallback_quantity = await self._get_inventory_fallback_quantity(
              product_id
            )
  ...
  async def _get_inventory_fallback_quantity(
    self, product_id: str
  ) -> int | None:
    ...
    product_qty = await db.get_product_inventory_quantity(
      self.products_session, product_id
    )
    if product_qty is None:
      return db.DEFAULT_INVENTORY_QUANTITY
    return product_qty
  ...
  async def _validate_inventory(
    ...
    for line in checkout.line_items:
      ...
      if qty_avail is None:
        qty_avail = await self._get_inventory_fallback_quantity(product_id)
