...
This repository already vendors both the Python SDK and the merchant server.
Install dependencies for the SDK first, then the server.

```shell
pushd packages/ucp-sdk-python
uv sync
popd
pushd legacy/merchant-server-python
uv sync
popd
```
...
```shell
mkdir /tmp/ucp_test
cd legacy/merchant-server-python
